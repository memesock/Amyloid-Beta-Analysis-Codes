...
%   at least I can't yet figure out how to get it to work. 
...
for clust = 0:(nClusters-1)
    ...
    %% Align structures that have minimum resulting RMSDs
        ...
        structure2 = whereToAlign(i,2);
        files_inThisCluster{structure1}
        files_inThisCluster{structure2}
        pdb1 = pdbread([folderName,'/PDBs_SpectralClustered/Cluster',num2str(clust),'/',files_inThisCluster{structure1}]);
        ...
        %save([folderName,'/PDBs_SpectralClustered/Cluster',num2str(clust),'/Aligned/',files_inThisCluster{k}], 'PDB2TX')
        %pdbwrite([folderName,'/PDBs_SpectralClustered/Cluster',num2str(clust),'/Aligned/',files_inThisCluster{structure2}], PDB2TX)
        
        pdbwrite([folderName,'/PDBs_SpectralClustered/Cluster',num2str(clust),'/',files_inThisCluster{structure2}], PDB2TX)
